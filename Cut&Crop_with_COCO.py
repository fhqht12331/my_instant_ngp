...
im = cv2.imread(glob.glob('images\*.jpg'))
...
for fname in images:
    ...
    cv2.imwrite("crop_images\*.jpg", im)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()

crop_images = glob.glob('crop_images\*.jpg')

imbg = cv2.imread("background.png")

for fname in crop_images:
    ...
    cropped = Image.fromarray(crop_images[y_min:y_max, x_min:x_max, :], mode='RGB')
    ...
    paste_position = (300, 150) #((x_max-x_min)/2, (y_max-y_min)/2)
